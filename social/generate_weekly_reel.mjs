...
function pageDurations(groups) {
  const defaultSeconds = groups.length === 1 ? 10.75 : 7;
...
// Bewusste Stil-Entscheidung (nicht "genauer", sondern optimistischer):
// GFS zeigte im Vergleich mit best_match/ECMWF am haeufigsten die
// sonnigste/trockenste Einschaetzung fuer denselben Ort+Tag. Reels sollen
// eher positiv wirken statt maximal vorsichtig — deshalb bewusst GFS statt
// Open-Meteos automatischer "best_match"-Auswahl (die am pessimistischsten
// war) oder eines Mehrheitsentscheids mehrerer Modelle.
const WEATHER_MODEL = 'gfs_seamless';
...
async function fetchWeatherForFestivals(festivals) {
  ...
  for (const [key, { lat, lng }] of byLocation) {
    try {
      const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&daily=weathercode,temperature_2m_max&timezone=Europe%2FBerlin&forecast_days=16&models=${WEATHER_MODEL}`;
...
// Monatsuebergreifende Spannen ("31. Juli–1. August") sind in der schmalen
// row-date-Spalte zu lang und wirken gequetscht — dort beide Monate auf 3
// Buchstaben kuerzen. Einzelmonats-Spannen ("24.–26. Juli") bleiben
// ausgeschrieben, die passen locker.
...
function pageHtml(group, pageNum, totalPages, weekLabel, yearLabel, dateRangeTitle, getWeather) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .row { display:flex; align-items:center; gap:22px; border:2px solid #ff2d00; border-radius:6px; background: rgba(255,45,0,0.06); padding: 20px 50px 20px 26px; margin-bottom: 16px; }
